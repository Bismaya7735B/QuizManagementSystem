...
def update_question(q_id, q_data):
    if "branch" in q_data: q_data["branch"] = normalize_course_name(q_data["branch"])
    supabase.table("questions").update(q_data).eq("id", q_id).execute()
                    ...
                    if missing_fields:
                        ...
                        if not branch_questions:
                            ...
                            if required_passkey and student_passkey.strip() != required_passkey and not existing_score:
                                ...
                                st.session_state.role = "candidate"
                                st.session_state.user_id = clean_roll
                                st.session_state.student_info = {"Name": name.strip(), "Roll No": clean_roll, "Section": section.strip(), "Branch": normalized_branch}
                                st.query_params["session_role"] = "candidate"
                                st.query_params["session_user"] = clean_roll
            ...
            with st.form("student_history_form"):
                ...
                if submit_history:
                    if not hist_roll.strip() or not hist_branch.strip():
                        ...
                        clean_branch = normalize_course_name(hist_branch)
                        rec = get_student_score(clean_roll)
                        if not rec: st.error(f"❌ No completed assessment record found for Roll Number '{clean_roll}'.")
                        else:
                            st.session_state.logged_in = True
                            st.session_state.role = "candidate"
                            st.session_state.user_id = clean_roll
                            st.session_state.student_info = {"Name": rec.get("name", "Student"), "Roll No": clean_roll, "Section": rec.get("section", ""), "Branch": clean_branch}
                            st.query_params["session_role"] = "candidate"
                            st.query_params["session_user"] = clean_roll
                            st.query_params["c_name"] = rec.get("name", "Student")
                            st.query_params["c_section"] = rec.get("section", "")
...
def admin_dashboard():
    ...
    with tab1:
        now_str = datetime.now().strftime("%I:%M:%S %p")
        c_head1, c_head2 = st.columns([3, 1])
        with c_head1: st.subheader("Student Submissions & Real-Time Analytics")
        with c_head2: st.caption(f"⏱️ Refreshed at: `{now_str}`")
        ...
        if not scores_data:
            ...
            branches = sorted(df["Course"].unique().tolist())
            selected_branch = st.selectbox("Filter Submissions by Course", ["All Courses"] + branches)
            if selected_branch != "All Courses": df = df[df["Course"] == selected_branch]
            st.dataframe(df, use_container_width=True)
            ...
            with st.form("delete_student_form"):
                ...
                if st.form_submit_button("Revoke & Allow Retake"):
                    if student_to_delete:
                        try:
                            delete_student_score(student_to_delete.split(" - ")[0])
                            st.toast(f"✅ Record removed.", icon="🗑️")
                            time.sleep(0.6)
                            st.rerun()
        ...
        with st.form("add_question_form"):
            ...
            if st.form_submit_button("💾 Save Question to Bank", type="primary"):
                if not q_text or not correct_input or not target_branch:
                    ...
                    options = [opt.strip() for opt in options_input.split(",")] if options_input else []
                    is_valid = True
                    if q_type == "MSQ": correct_ans = [ans.strip() for ans in correct_input.split(",")]
                    elif q_type == "Numerical":
                        try: correct_ans = float(correct_input)
                        except ValueError:
                            st.error("❌ For Numerical questions, specify ONLY a valid numeric value.")
                            is_valid = False
                    else: correct_ans = correct_input.strip()

                    if is_valid:
                        new_question = {"branch": normalize_course_name(target_branch), "text": q_text, "type": q_type, "marks": marks, "options": options, "correct": correct_ans, "unit": q_unit.strip()}
                        try:
                            add_question(new_question)
                            st.toast(f"✅ Question saved!", icon="🎉")
                            time.sleep(0.6)
                            st.rerun()
                        except Exception as e: st.error(f"Database error: {e}")
                ...
                for idx, q in enumerate(branch_questions):
                    with st.expander(f"Item #{idx + 1}: {q['text'][:65]}... ({q['type']} | {q['marks']} Marks)"):
                        with st.form(key=f"edit_form_{q['id']}"):
                            edit_q_text = st.text_area("Question Text", value=q['text'])
                            col_a, col_b, col_c = st.columns(3)
                            with col_a: edit_q_type = st.selectbox("Type", ["MCQ", "MSQ", "Numerical"], index=["MCQ", "MSQ", "Numerical"].index(q['type']))
                            with col_b: edit_marks = st.selectbox("Marks", [1, 2], index=[1, 2].index(q['marks']))
                            with col_c: edit_unit = st.text_input("Unit", value=q.get('unit', ''))
                            edit_options = st.text_input("Options (Comma-separated)", value=",".join(q.get('options') or []))
                            c_val = ",".join(q['correct']) if q['type'] == 'MSQ' and isinstance(q['correct'], list) else str(q['correct'])
                            edit_correct = st.text_input("Correct Answer", value=c_val)

                            c1, c2 = st.columns(2)
                            with c1: save_edit = st.form_submit_button("💾 Update Changes", type="primary")
                            with c2: delete_q = st.form_submit_button("🗑️ Delete Question")

                            if save_edit:
                                is_valid = True
                                if edit_q_type == "MSQ": parsed_correct = [ans.strip() for ans in edit_correct.split(",")]
                                elif edit_q_type == "Numerical":
                                    try: parsed_correct = float(edit_correct)
                                    except ValueError:
                                        st.error("❌ For Numerical questions, specify ONLY a valid numeric value.")
                                        is_valid = False
                                else: parsed_correct = edit_correct.strip()

                                if is_valid:
                                    updated_payload = {"text": edit_q_text, "type": edit_q_type, "marks": edit_marks, "options": [opt.strip() for opt in edit_options.split(",")] if edit_options else [], "correct": parsed_correct, "unit": edit_unit.strip()}
                                    try:
                                        update_question(q['id'], updated_payload)
                                        st.toast("✅ Question updated successfully!", icon="💾")
                                        time.sleep(0.6)
    ...
    with tab4:
        ...
        if not existing_branches:
            ...
            all_cfg = get_all_settings()
            if all_cfg:
                formatted_cfg = [{"Course / Branch": normalize_course_name(c.get("branch", "")), "Time Limit (Mins)": c.get("time_limit", 30), "Active Passkey": c.get("passkey", "")} for c in all_cfg]
                df_cfg = pd.DataFrame(formatted_cfg).drop_duplicates(subset=["Course / Branch"])
                st.dataframe(df_cfg, use_container_width=True)
    ...
    with tab5:
        ...
        if existing_branches:
            branch_to_delete = st.selectbox("Select Course Assessment Suite to Delete", existing_branches)
            if st.button("🗑️ Permanently Delete Course Assessment"):
                if st.checkbox(f"I understand this deletes all questions, scores, and settings for {branch_to_delete}"):
                    try:
                        delete_branch_data(branch_to_delete)
                        st.toast(f"✅ Deleted all records for {branch_to_delete}.", icon="🗑️")
                        time.sleep(0.6)
                ...
                else: st.error("Please confirm the deletion checkbox first.")

        st.divider()
        st.markdown("#### 🚨 Total System Purge")
        if st.button("🚨 Wipe Entire Portal Database"):
            if st.checkbox("Confirm complete system wipe (Deletes all branches, question banks, and submissions)"):
                try:
                    wipe_all_data()
                    st.toast("✅ System database completely cleared.", icon="🚨")
                    time.sleep(0.6)
            ...
            else: st.error("Please confirm the wipe checkbox first.")
...
def candidate_dashboard():
    ...
    with col1:
        st.markdown(f"<h2 style='margin:0;'>📝 Examination: {html.escape(student_branch)}</h2>", unsafe_allow_html=True)
        st.markdown(f"<div style='font-size: 0.95rem; color: #475569; margin-top: 4px;'>Candidate: <b>{html.escape(st.session_state.student_info.get('Name', ''))}</b> &nbsp;|&nbsp; Roll ID: <code>{html.escape(roll_no)}</code> &nbsp;|&nbsp; Section: <b>{html.escape(st.session_state.student_info.get('Section', ''))}</b></div>", unsafe_allow_html=True)
    with col2: st.button("🚪 Exit", on_click=logout, use_container_width=True)
    ...
    # -------------------------------------------------------------
    # RESULT SCREEN - SHOW RED BANNER IF MALPRACTICE DETECTED
    # -------------------------------------------------------------
    if student_record is not None:
        ...
        student_responses = parse_saved_responses(student_record.get("responses"))
        filter_choice = st.radio("Filter Questions:", ["All Items", "Correct Only (✔)", "Incorrect Only (❌)", "Unattempted Only (⚪)"], horizontal=True)

        for i, q in enumerate(my_questions):
            user_ans = student_responses.get(str(i))
            if user_ans is None and q.get("id"): user_ans = student_responses.get(str(q.get("id")))
            q_type = q.get("type", "MCQ")
            marks = q.get("marks", 1)
            correct_key = q.get("correct")
            ...
            is_correct = False
            if not is_unattempted:
                if q_type == "MCQ": is_correct = (str(user_ans).strip() == str(correct_key).strip())
                elif q_type == "MSQ": is_correct = (set(str(u).strip() for u in (user_ans if isinstance(user_ans, list) else [user_ans])) == set(str(c).strip() for c in (correct_key if isinstance(correct_key, list) else [correct_key])))
                elif q_type == "Numerical":
                    try: is_correct = (float(user_ans) == float(correct_key))
                    except: is_correct = False

            if filter_choice == "Correct Only (✔)" and not is_correct: continue
            if filter_choice == "Incorrect Only (❌)" and (is_correct or is_unattempted): continue
            if filter_choice == "Unattempted Only (⚪)" and not is_unattempted: continue
            ...
            if is_unattempted:
                ...
                box_class = "review-box review-box-wrong"
                status_badge = f"<span class='badge-wrong'>❌ Incorrect (-{marks * 0.25 if q_type == 'MCQ' else 0} Marks Penalty)</span>"

            st.markdown(f"<div class='{box_class}'><div class='question-text-content'><b>Q{i + 1}.</b> {html.escape(str(q.get('text', '')))} <span class='badge-mark'>{marks} Marks</span> {status_badge}</div>", unsafe_allow_html=True)
            ...
            if q_type in ["MCQ", "MSQ"]:
                for opt in (q.get("options") or []):
                    ...
                    if q_type == "MCQ":
                        ...
                        is_this_correct_key = (opt_str in [str(c).strip() for c in (correct_key if isinstance(correct_key, list) else [correct_key])])
                        is_this_user_pick = (opt_str in [str(u).strip() for u in (user_ans if isinstance(user_ans, list) else [user_ans])] if not is_unattempted else False)

                    if is_this_correct_key and is_this_user_pick:
                        st.markdown(f"<div class='option-item option-correct-key'><span>{html.escape(opt_str)}</span><span><span style='font-size:0.8rem; font-weight:700; color:#166534;'>✔ Your Choice (Correct Key)</span></span></div>", unsafe_allow_html=True)
                    elif is_this_correct_key and not is_this_user_pick:
                        st.markdown(f"<div class='option-item option-correct-key'><span>{html.escape(opt_str)}</span><span><span style='font-size:0.8rem; font-weight:700; color:#166534;'>✔ Official Correct Key</span></span></div>", unsafe_allow_html=True)
                    elif not is_this_correct_key and is_this_user_pick:
                        st.markdown(f"<div class='option-item option-user-wrong'><span>{html.escape(opt_str)}</span><span><span style='font-size:0.8rem; font-weight:700; color:#991b1b;'>❌ Your Choice (Incorrect)</span></span></div>", unsafe_allow_html=True)
                    else:
                        st.markdown(f"<div class='option-item option-default'><span>{html.escape(opt_str)}</span><span></span></div>", unsafe_allow_html=True)
            elif q_type == "Numerical":
                unit_str = f" {html.escape(q.get('unit', ''))}" if q.get("unit") else ""
                st.markdown(f"""
                <div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:10px; padding:12px 16px; margin-top:8px;">
                    <div style="display:flex; justify-content:space-between; margin-bottom:6px;"><span style="color:#475569; font-weight:600;">Your Submitted Answer:</span><b>{html.escape(str(user_ans)) if not is_unattempted else "<span style='color:#64748b;'>Not Attempted</span>"}{unit_str}</b></div>
                    <div style="display:flex; justify-content:space-between; border-top:1px dashed #cbd5e1; padding-top:6px;"><span style="color:#166534; font-weight:600;">Official Correct Key:</span><b style="color:#166534;">{html.escape(str(correct_key))}{unit_str}</b></div>
                </div>
                """, unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)
    ...
    try {{
        ...
        function showWarning() {{
            try {{
                ...
                pDoc.getElementById('p-warn-btn').onclick = function() {{ div.remove(); }};
            }} catch(e) {{}}
        }}
        ...
        function handleVisChange() {{
            let isHidden = false;
            try {{ isHidden = document.hidden || window.parent.document.hidden; }} catch(e) {{ isHidden = document.hidden; }}
    ...
    def finalize_normal_exam(user_ans_dict, is_late=False, is_violation=False):
        ...
        for i, q in enumerate(my_questions):
            ...
            is_correct = False
            if q["type"] == "MCQ": is_correct = (str(ans).strip() == str(q["correct"]).strip())
            elif q["type"] == "MSQ": is_correct = (set(str(c).strip() for c in (q["correct"] if isinstance(q["correct"], list) else [q["correct"]])) == set(str(a).strip() for a in (ans if isinstance(ans, list) else [ans])))
            elif q["type"] == "Numerical":
        ...
        # THIS FLAGS IT FOR THE ADMIN SCORE ANALYTICS BOARD
        if is_violation: status_text = "🚨 Forced Submitted (Malpractice)"
        elif is_late: status_text = "⚠️ Rejected (Late Submission)"
        else: status_text = "✅ Completed"

        final_record = {"roll_no": roll_no, "name": st.session_state.student_info.get("Name", ""), "section": st.session_state.student_info.get("Section", ""), "branch": student_branch, "score": score, "correct": correct_count, "wrong": wrong_count, "deducted": total_deducted, "responses": saved_responses, "status": status_text}
        try:
            save_student_score(final_record)
            time.sleep(0.4)
            st.rerun()
